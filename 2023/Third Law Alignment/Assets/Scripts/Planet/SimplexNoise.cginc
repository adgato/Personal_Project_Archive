...
void snoise_float(float3 coord, out float value)
{
    value = 0.5 * snoise(coord) + 0.5;
}
void snoise_half(half3 coord, out half value)
{
    value = 0.5 * snoise(coord) + 0.5;
}

void make_path_snoise_float(float3 uvw, float offset, float scale, float pathWidth, uint Layers, out float value)
{
    float noise = 1;
    for (uint i = 0; i < Layers; i++)
    {
        float3 randomOffset = i * float3(snoise(i + float3(5, -5, 0)), snoise(i + float3(-5, 5, 0)), snoise(i + float3(0, 0, 5)));
        noise = min(noise, abs(snoise(scale * uvw + offset + randomOffset) / pathWidth));
    }

    value = saturate(1 - noise * noise);
}
